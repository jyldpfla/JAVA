...
public class Library {
	...
	public Book[] getUpSeq() {
		...
		for (int i = 0; i < books.length - 1; i++) {
			for (int j = i + 1; j < books.length; j++) {
				if (books[i].getPrice() > books[j].getPrice()) {
					temp = books[i]; // j값이 들어가면 i값이 사라지므로 다른 곳에 넣어주기
					books[i] = books[j]; // i값에 j값 넣어서 순서 앞으로 오도록 정렬
					books[j] = temp;
				} else if (books[i].getPrice() == books[j].getPrice()) {
					if (books[i].getTitle().length() > books[j].getTitle().length()) {
						temp = books[i];
	...
	public void searchType() {
		...
		while (true) {
			...
			for (int i = 0; i < books.length; i++) {
				if (books[i].getType().equals(search)) {
					count++; // 같은 가격이 나왔을 때 개수+해서 배열의 길이 = count로 해서 길이가 늘어남
				}
			}
			if (count > 0) {
				break;
		...
		temp = new Book[count]; // 배열의 길이 변수로 조정
		int num = 0;
		for (int i = 0; i < books.length; i++) {
			if (books[i].getType().equals(search)) {
				temp[num] = books[i]; // index번호 같으면 안되므로 num이란 변수 추가 생성
				num++;
			}
		}
	...
	public void addBook() {
		...
		while(true) {
			books = Arrays.copyOf(books, books.length + 1); // (copy할 배열 , 배열의 길이)
			// 책 1권만큼의 공간을 위해 +1
